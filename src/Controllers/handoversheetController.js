...
const gethandoversheetdata = async function (req, res) {
  try {
    ...
    res
      .status(200)
      .json({ message: "Data fetched successfully", Data: gethandoversheet });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
...
const edithandoversheetdata = async function (req, res) {
  ...
  try {
    ...
    if (!updatedHandover || !updatedProject) {
      return res
        .status(404)
        .json({ message: "Record not found for provided code" });
    }
...
//edit for bd
const updatehandoverbd = async function (req, res) {
  try {
    const { p_id,
         customer_details,
      order_details,
    ...
    const updatedHandover = await hanoversheetmodells.findOneAndUpdate(
      {
        p_id: p_id,
        status_of_handoversheet: { $in: ["reject"] },
      },
      {
        $set: {
          customer_details,
          ...
          invoice_detail,
          submitted_by,
          status_of_handoversheet: "draft", // or whatever status you want to set
        },
      },
...
// status of handover sheet not in use for latest flow
const updateStatusOfHandoversheet = async function (req, res) {
  try {
    ...
    if (user.role !== "admin" && user.role !== "superadmin") {
      return res
        .status(403)
        .json({ message: "Only admin or superadmin can update the status" });
    }

    // Update handover status not in use for latest flow
    const updatedHandover = await hanoversheetmodells.findOneAndUpdate(
      { p_id: p_id },
    ...
    if (!updatedHandover) {
      return res
        .status(404)
        .json({ message: "Handover sheet not found for given project ID" });
    }

    res.status(200).json({
      message: "Status updated successfully",
      Data: updatedHandover.status_of_handoversheet,
    });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
const updateStatusHandoversheet = async function (req, res) {
  try {
    ...
    if (!status) {
      return res
        .status(400)
        .json({ message: "This handover sheet is cannot be edited." });
    }
    // Update handover sheet
    ...
    res
      .status(200)
      .json({ message: "Status updated successfully", Data: status });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
