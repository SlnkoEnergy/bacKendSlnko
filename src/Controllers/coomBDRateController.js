...
const addCommBDRate = async function (req, res) {
  try {
    ...
    res
      .status(200)
      .json({ msg: "Comm Rate added successfully", commBDRate: commBDRate });
  } catch (error) {
...
const editCommBDRate = async function (req, res) {
  try {
    ...
    let data = await CommBDRate.findByIdAndUpdate(_id, updateData, {
      new: true,
    });
    if (!data) {
    ...
    return res
      .status(200)
      .json({ msg: "Comm Rate updated successfully", data: data });
  } catch (error) {
    return res.status(500).json({ msg: error.message });
  }
};
...
const getCommBdRateHistory = async function (req, res) {
  try {
    ...
    return res
      .status(200)
      .json({ msg: "BD Rate History Log", data: commBdRateHistory });
  } catch (error) {
    return res.status(500).json({ msg: error.message });
  }
};
