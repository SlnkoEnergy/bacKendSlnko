...
const editOffer = async function (req, res) {
  try {
    ...
    let data = await conmmOfferModells.findByIdAndUpdate(_id, updateData, {
      new: true,
    });
    if (!data) {
    ...
    return res
      .status(200)
      .json({ msg: "Offer updated successfully", data: data });
  } catch (error) {
    return res.status(500).json({ msg: error.message });
  }
};
...
const deleteOffer = async function (req, res) {
  try {
    ...
    return res
      .status(200)
      .json({ msg: "Offer deleted successfully", data: data });
  } catch (error) {
    return res.status(500).json({ msg: error.message });
  }
};
